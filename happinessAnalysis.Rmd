...
```
We need to verify our assumption,as to our these factors actually detrminants of someone's happiness in life.
...
plot(logitModel$residuals,logitModel$fitted.values)
...
clusplot(kmeansCluster[c("Adrenaline.sports","Active.sport","Health")],kmeansCluster$clusterNo)
...
clusplot(kmeansCluster[c(colnames(trainData1))],kmeansCluster$clusterNo)
table(kmeansCluster$Darkness,kmeansCluster$clusterNo)
```
...
tempdf <- kmeansCluster %>%
  ...
  p <- ggplot(tempdf, aes(fill=!!sym(factorV), y = counts,x=clusterNo)) +
    ...
    theme(plot.title = element_text(size=16, margin=margin(13,100,13,0)))
  print(p)
}
...
# testData2$PredictedKmean[testData2$PredictedKmean ==  ] = 3
testData2$PredictedKmean2[testData2$PredictedKmean2 == 3] = 1
confusionMatrix(factor(testData2$happinessModified,levels = 0:1),factor(testData2$PredictedKmean2,levels = 0:1))
...
Hierarchical clustering is an approach for identifying clustering in the dataset by using pairwise distance matrix between observations as clustering criteria.
...
Here also we classify into 2 clusters by taking k=2 and we use the "ward.D" method.
```{r}
...
The accuracy is obtained by comparing the actual and predicted cluster values of Test data.
...
confusionMatrix(factor(resultsDF$predictedKMedian,levels = 0:1),factor(testData1$Happy ,levels = 0:1))
#Accuracy=68.69%
```
...
determineMajority <- function(){
  ...
  {
    if(rowSums(resultsDF[i,][c(2,4:6)]) >= 2){
