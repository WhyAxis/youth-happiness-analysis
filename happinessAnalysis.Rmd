...
nreqAttributes <- names(rawData1) %in% c("Height","Weight","Gender")
...
set.seed(123)
km.res <- kmeans(trainData1, 3, nstart = 25)
km.res$centers
...
testData1$Predicted = predictedCluster
testData1$Predicted[testData1$Predicted == 1| testData1$Predicted == 2] = 0
testData1$Predicted[testData1$Predicted == 3] = 1
confusionMatrix(factor(actualData1$Happy,levels = 0:1),factor(testData1$Predicted,levels = 0:1))
```
...
happyLevels <- rev(levels(as.factor(rawData1[,17])))
...
labels_colors(dend) <-
  rainbow_hcl(2)[sort_levels_values(
    as.numeric(rawData1[,17])[order.dendrogram(dend)]
  )]

labels(dend) <- paste(as.character(rawData1[,17])[order.dendrogram(dend)],"(",labels(dend),")", sep = "")
...
fit <- rpart(Happy~Health+Changing.the.past+Public.speaking+Interests.or.hobbies+Storm+Darkness+Spiders+Fear.of.public.speaking+Cars+Active.sport+Adrenaline.sports+Romantic+Western+Action+bmi, data = trainData1, method = 'class')
...
set.seed(123)
pam.res <- pam(trainData1, 5)
pam.res$medoids

fviz_cluster(pam.res, data = trainData1)
predictedClusterPam <- as.vector(cl_predict(pam.res,testData1))

testData1$PredictedPam = predictedClusterPam
testData1$PredictedPam[testData1$PredictedPam == 1 |testData1$PredictedPam == 4 | testData1$PredictedPam == 5] = 0
testData1$PredictedPam[testData1$PredictedPam == 2| testData1$PredictedPam == 3] = 1
confusionMatrixPam = confusionMatrix(factor(testData1$PredictedPam,levels = 0:1),factor(testData1$Happy,levels = 0:1))
confusionMatrixPam
...
```{r}
# set.seed(5)
# kMedian = kcca(kMedianData, k=4, family=kccaFamily("kmedians"), save.data=TRUE)
# centroids = parameters(kMedian)

#pred_cluster<- predict(kMedian, k=4, kccaFamily("kmedians"))
# 
# copy_test<-test_data
# copy_test<-cbind(copy_test,pred_cluster)
# 
# 
# pred_time<-c()  #To store the centroid values for each data point belonging to a cluster
# 
# #To append centroid values based on the cluster predict for the data point 
# for (i in copy_test$pred_cluster)
# {
#     pred_time<-c(pred_time,centroids[i,])
# }
# copy_test<-cbind(copy_test,pred_time)
# 
# n_test<-nrow(test_data)
# diff<-c() #To store difference between actual and predicted values
# for (i in 0:n_test)
# {
#   #To calculate difference between actual value and predicted value when k=6
#   calc_diff<-abs(copy_test$`Absenteeism time in hours`[i]- copy_test$pred_time[i])
#   diff<-c(diff,calc_diff)
# }
# copy_test<-cbind(copy_test,diff) #Append difference values to the file
# View(copy_test)
```
...
```{r}
#predictedCluster <- as.vector(cl_predict(km.res,testData1))
copyTestData1$Predicted = predClusterMedian
copyTestData1$Predicted[copyTestData1$Predicted == 2| copyTestData1$Predicted == 4] = 0
copyTestData1$Predicted[testData1$Predicted == 1| copyTestData1$Predicted == 3] = 1

confusionMatrix(factor(copyTestData1$Happy,levels = 0:1),factor(copyTestData1$Predicted,levels = 0:1))
```
