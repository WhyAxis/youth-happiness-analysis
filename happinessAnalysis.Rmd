...
resultsDF$PredictedClusterVote = determineMajority(resultsDF[c(2,9:11)],3)
resultsDF$PredictedVote = determineMajority(resultsDF[c(2,4,5,8,11)],3)
...
```{r}
importantResults <- resultsDF[c(2,3,4,6,9,10,11)]
...
corrplot(cov(importantResults),method = "number",title = "Covariace Between Different Models",mar = c(0,0,2,0))
```
...
wAverageDetermination <- function(){
  ...
  {
    resultsDF$PredictedWA[i] = wAverageRow(resultsDF[i,][c(2,3,4,6,9,11)])
